<?php
  ...
  if (isset($_POST["submit"])){
    ...
    $stu_id = generateStudentID($mysqli);
    /* Mother Record */
   ...
   // if(move_uploaded_file($tmpName,"../images/".$stu_id.".".$ext))
	...
	$q = "select floor(DATEDIFF(NOW(),DOB)/365.25) AS STUAGE from student";
...
echo $stu_co;
	...
	$query_run1= $mysqli->query($q) or die ($mysqli->error);

    $q = "INSERT INTO Student (Stu_ID,Fname,Lname,dob,gender,father_cnic,guardian_cnic,stu_image,reg_date,classname) VALUES ('$stu_id','$stu_fname', '$stu_lname', timestamp('$stu_dob'), '$stu_gender',$father_cnic,$g_cnic,'$ext', CURRENT_DATE, '$stu_class')";
	$query_run2= $mysqli->query($q) or die ($mysqli->error);
  ...
  function generateStudentID($mysqli){

    $id  = date("Y")%2000;
    $id = $id."I-";

    $q = "SELECT * FROM ( SELECT * FROM Student WHERE STU_ID LIKE '$id%' ORDER BY STU_ID DESC) AS ST";
    $query_run= $mysqli->query($q) or die ($mysqli->error);
    
    $row;

    if (($row = $query_run->fetch_assoc()) == false){
      $num = "1000";
      $id = $id.$num;
      return $id;
    }
